...
if convert_full_vault:
    try: 
        ...
        filenames = [str(p).replace(f"{args.input}/", "") for p in Path(args.input).rglob('*') if p.is_file() and not p.name.startswith('.') and not str(p).startswith('.') and str(p.parents[-1]) != 'Templates']
